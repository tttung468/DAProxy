...
DWORD WINAPI Exe(LPVOID lpParam)
{
	...
	checkURL = CheckURLinList(URL, lenURL);		//kiểm tra URL này đã được caching chưa
	if (checkURL == FALSE)		//chưa đc caching
	{
		...
		else
		{
			...
			if (Check200OK == true)
			{	
				ofstream CachingFile;		//tạo file để caching

				CachingFile.open(URL_fileName, ios::out);
				CachingFile << headerResponse;

				fstream URLList;
				URLList.open("WebCache\\\\URLList.txt", ios::app);		//Cập nhật URL đã được caching
			...
			do
			{
				...
				if (iResult < 512)	//Kết thúc HTML			
				{
					...
					if (Check200OK == true)
					{
						fstream CachingFile;		//tạo file để caching
						CachingFile.open(URL_fileName, ios::app);
						CachingFile << HTMLResponse;
						CachingFile.close();
					}
					break;
				}
